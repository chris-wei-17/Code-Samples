...
void drawFeaturePoints(cv::Mat image, std::vector<cv::Point2f>& points)
{
    ...
    {
        circle(image, cv::Point(points[i].x, points[i].y), radius, CV_RGB(0,0,255));
    }
}
...
void display(int frame_id, cv::Mat& trajectory, cv::Mat& pose, std::vector<Matrix>& pose_matrix_gt, float fps, bool show_gt)
{
    ...
    int y = int(pose.at<double>(2)) + 100; // this is y axis for display, but is actual z translation - FORWARD 

    circle(trajectory, cv::Point(x, y) ,1, CV_RGB(255,0,0), 2);
      
    //float tx = trans.at<float>(0,1);
   // std::string stx = std::to_string(tx);
      
      
    //putText(trajectory, stx, cv::Point(100, 1100), 1, 2, CV_RGB(255,0,0), 2);
...
double integrateOdometryStereo(int frame_i, cv::Mat& rigid_body_transformation, cv::Mat& frame_pose, const cv::Mat& rotation, const cv::Mat& translation_stereo, double& scale)
{
    ...
    // Uncomment to check rigid body transform calculation
    //std::cout <<"RIGID TRANSFORM INTERMEDIATE"<<rigid_body_transformation;
    
    cv::vconcat(rigid_body_transformation, addup, rigid_body_transformation);

    // std::cout << "rigid_body_transformation" << rigid_body_transformation << std::endl;

    scale = sqrt((translation_stereo.at<double>(0))*(translation_stereo.at<double>(0)) 
    ...
    {
     std::string warning = "[WARNING] scale below 0.001, or incorrect translation";
