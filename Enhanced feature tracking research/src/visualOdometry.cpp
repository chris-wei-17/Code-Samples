                      ...
                      int& kCorners)
{
    ...
    {
		std::cout << "NUMBER OF VO FEATURES: " << currentVOFeatures.size() << std::endl;
        // append new features with old features
        appendNewFeatures(imageLeft_t0, currentVOFeatures, kCorners);   
    ...
    // --------------------------------------------------------
    int bucket_size = imageLeft_t0.rows/25;
    std::cout << "NUMBER OF IMAGE ROWS: " << imageLeft_t0.rows << std::endl;
    int features_per_bucket = 1;
    bucketingFeatures(imageLeft_t0, currentVOFeatures, bucket_size, features_per_bucket);
	std::cout << "NUMBER OF VO FEATURES (size >= k): " << currentVOFeatures.size() << std::endl;
                         ...
                         bool mono_rotation)
{
      ...
      if(mono_rotation)
      {
      	...
      	std::cout << "USING MONO ROTATION" << std::endl;
      }
      // ------------------------------------------------
      ...
      cv::Mat intrinsic_matrix = (cv::Mat_<float>(3, 3) << projMatrl.at<float>(0, 0), projMatrl.at<float>(0, 1), projMatrl.at<float>(0, 2),
                                                   ...
                                                   projMatrl.at<float>(2, 0), projMatrl.at<float>(2, 1), projMatrl.at<float>(2, 2)); // changed, verify 8/24/20
	  //std::cout << "Intrinsic Matrix: " << intrinsic_matrix << std::endl;
      ...
      /////// maybe trim to 'k' points here 

      //#if 1 remove CUDA/NonCUDA condition
      cv::Mat inliers; 
      ...
      if (!mono_rotation)
      {
        ...
        std::cout << "USING RODRIGUES ROTATION" << std::endl;
      }

      std::cout << "[trackingFrame2Frame] inliers size: " << inliers.size() << std::endl;
	  //std::cout << "Rodrigues rotation" << rotation << std::endl;
}
                     ...
                     double& scale)
{
      ...
      {
          cv::circle(vis, cv::Point(pointsLeft_t0[i].x, pointsLeft_t0[i].y), 8, CV_RGB(0,255,0));
      }
      ...
      for (int i = 0; i < pointsLeft_t1.size(); i++)
      {
          cv::arrowedLine(vis, pointsLeft_t0[i], pointsLeft_t1[i], CV_RGB(0,0,255), 3);
      }

      // Overlay fps onto image output
      cv::putText(vis, camera_fps, cv::Point(50,50) , 1, 2, CV_RGB(255,0,0), 2);

	  if (scale < 0.001)
	  {
	  // Show scale calc
	  cv::putText(vis, "scale less than 0.001" , cv::Point(50,100) , 1 , 2 , CV_RGB(255,0,0), 2);
	  }

      cv::imshow("vis ", vis );  
      
      std::cout << "DISPLAY SCALE: " << scale << std::endl ;
