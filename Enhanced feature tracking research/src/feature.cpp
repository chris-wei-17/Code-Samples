...
void featureDetectionGoodFeaturesToTrack(cv::Mat image, std::vector<cv::Point2f>& points, int& kCorners)  
{   
//uses GoodFeaturesToTrack for feature dection, modify parameters as necessary

  int maxCorners = kCorners;
  ...
  std::cout << " ------ MAX number of corners: " << maxCorners << " ------" << std::endl;
  std::cout << " ------ number of corners: " << points.size() << " ------" << std::endl;
}
...
void circularMatching(cv::Mat img_l_0, cv::Mat img_r_0, cv::Mat img_l_1, cv::Mat img_r_1,
  ...
  // Put optical flow block into if condition due to features being removed after age > 10
  // This allows for a lower VO size as well at idle without feeding empty vector to LK flow
  ...
  else if (current_features.size() == 0)
  {
...
void appendNewFeatures(cv::Mat& image, FeatureSet& current_features, int& kCorners)
{
    std::vector<cv::Point2f>  points_new;
    featureDetectionGoodFeaturesToTrack(image, points_new, kCorners);
